...
<mark>围绕周期趋势，采用动量（momentum）+ 反转（reverse）组合的方式，动量交易占主导，反转交易进行试错。</mark>

<mark>捕捉市场动量与反转共识，主要依靠分析盈利和估值双因素在市场价格驱动力中的贡献比例，通过大类资产价格联动的相关性分析和因果逻辑分析，定位市场当前状态所处的周期位置区间。</mark>
...
由同一因子驱动的价格变化在不同标的上会存在分化，此时是区分该因子的市场共识与不同标的中因子占比的关键。如果是动量因子，关键是选择因子占比相对高的标的；如果是反转因子，关键需要区分该因子是否形成市场主力资金的共识。

## 有效交易时间的资金流动分析

全球金融市场已经处于无间断运转状态，但是交易量与流动性的时间分布有显著的“峰谷”特征，主要分为 `Asia` `Europe` `America` 三个时间段。由于金融市场的定价合理性与交易量与流动性呈显著的正相关，因此只有交易峰值时间段的撮合价格才具有参考意义（其余交易时间段的价格通常可以视为噪声干扰）。由于不同交易峰值时间段的主力资金来源通常是固定的，因此通过不同时间段的价格变化可以分析不同主力资金的动机区别。

![](./img/Trading-Volume-by-Session.png)
