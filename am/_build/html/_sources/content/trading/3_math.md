...
对于函数 f，其导函数 f' 的波动率通常更高，其原函数 F 的波动率通常更低。因此最佳方式是观察 f' 和 f 的变化，从中提取领先信号，用于交易噪声更低的 F。

## 正交分析

为了理解一组向量 v 中的真实成分构成，需要对其进行正交分析，提取出 v 中所有的正交分量。

## 概况分布

正态分布是应用频率最高的概率分布模型，但是偏态分布模型同样有广泛应用。误将偏态分布问题建模成正态分布模型将会导致严重的后果。
